...
def clear_file(filename):
    with open(filename, 'w') as file:
        pass  # Оставляем файл пустым
def update_ui():
    clear_file('data2.txt')
    """Функция для обновления текста на интерфейсе"""
    file = open('data2.txt', 'a')
    obt_frame = vessel.orbit.body.non_rotating_reference_frame
    while not ui_thread_stop:
        try:
            file.write(f'{(vessel.flight(srf_frame).speed)};{(vessel.flight(srf_frame).surface_altitude)};{vessel.position(srf_frame)[0]};{vessel.position(srf_frame)[1]};{vessel.position(srf_frame)[2]}\n')
            file.flush()
            time.sleep(1)  # Обновляем интерфейс каждые 100 мс
        except Exception as e:
            print(f"UI update thread error: {e}")
            continue
    file.close()
def update_ui2():
    clear_file('data22.txt')
    """Функция для обновления текста на интерфейсе"""
    file2 = open('data22.txt', 'a')
    obt_frame = vessel.orbit.body.non_rotating_reference_frame
    while not ui_thread_stop:
        try:
            file2.write(f'{(vessel.flight(srf_frame).speed)};{(vessel.flight(srf_frame).surface_altitude)};{vessel.position(srf_frame)[0]};{vessel.position(srf_frame)[1]};{vessel.position(srf_frame)[2]}\n')
            file2.flush()
            time.sleep(1)  # Обновляем интерфейс каждые 100 мс
        except Exception as e:
            print(f"UI update thread error: {e}")
...
def time_to_target_height(h0, h_target, g_luna=1.625, initial_velocity=0):
    """
    Рассчитать время падения на Луну до целевой высоты.
    :param h0: начальная высота (м)
    :param h_target: целевая высота (м)
    :param g_luna: ускорение свободного падения на Луне (по умолчанию 1.62 м/с^2)
    :param initial_velocity: начальная скорость (по умолчанию 0 м/с)
    :return: время падения в секундах
    """
    if h_target > h0:
        raise ValueError("Целевая высота должна быть меньше начальной высоты.")
    ...
    if discriminant < 0:
        raise ValueError("Нет реального решения. Проверьте входные параметры.")

    # Берём только положительное значение времени
...
if __name__ == "__main__":
    ...
    ui_thread_stop = False
    time.sleep(0.25)
    ui_thread2 = threading.Thread(target=update_ui2, daemon=True)
    ui_thread2.start()
    # Дождитесь завершения ориентации
    vessel.control.sas = False
    vessel.control.rcs = False
